...
# Define a long short-term memory (LSTM) neural network class for sequence classification
class LongShortTermMemoryNeuralNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        super(LongShortTermMemoryNeuralNetwork, self).__init__()  # Call parent class constructor
        self.hidden_size = hidden_size  # Store hidden size for later use
        self.num_layers = num_layers  # Store number of layers
        # LSTM layer: processes sequences with specified input size, hidden size, and layers
    ...
    def forward(self, x):
        ...
        # Forward pass through LSTM: returns output and final hidden/cell states (we ignore with _)
...
# Initialize the LSTM neural network model and move to device
model = LongShortTermMemoryNeuralNetwork(input_size, hidden_size, num_layers, num_classes).to(device=device) 
