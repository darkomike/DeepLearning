...
# Define a gated recurrent unit (GRU) neural network class for sequence classification
class GatedRecurrentUnitNeuralNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        super(GatedRecurrentUnitNeuralNetwork, self).__init__()  # Call parent class constructor
        self.hidden_size = hidden_size  # Store hidden size for later use
        self.num_layers = num_layers  # Store number of layers
        # GRU layer: processes sequences with specified input size, hidden size, and layers
    ...
    def forward(self, x):
        ...
        # Forward pass through GRU: returns output and final hidden state (we ignore hidden state with _)
        out, _ = self.gru(x, h0)
...
model = GatedRecurrentUnitNeuralNetwork(input_size, hidden_size, num_layers, num_classes).to(device=device) 
